...
const Header = ({ isOpen, toggleClose, toggleOpen }) => {
  return (
    <div>
      <div className='fixed z-50 flex justify-between text-white bg-black sm1:hidden md:block'>
        <Link href="/" className='fixed mt-12 ml-20 text-sm tracking-tighter border-0 w-fit hover:text-gray-300 hover:text-xl'>
        ...
        <div className='fixed right-0 flex justify-between mt-12 mr-20 border-0 w-fit'>
          ...
          <div className='text-sm ml-28'>
            ...
            <div><Link href="https://github.com/zklou" target="_blank" rel="noreferrer" className="hover:text-gray-300">Github</Link></div>
            <div><Link href="https://www.linkedin.com/in/zhengkun-lou/" target="_blank" rel="noreferrer" className="hover:text-gray-300">LinkedIn</Link></div>
          </div>
        </div>
      </div>
      <div>
        <Link href="/" className='fixed z-50 mt-12 text-sm tracking-tighter text-white border-0 sm1:ml-4 md:ml-20 w-fit'>
          <div>Zhengkun Lou</div>
          <div>Full Stack Developer</div>
        </Link>
        <div className='fixed right-0 z-50 mt-12 mr-2 text-white'>
          <div className="sm1:block md:hidden">
            <div className='p-0 -mt-2 bg-white border-0 w-fit rounded-4xl'>
              <Hamburger color={'#0D0D0D'} toggled={isOpen} toggle={isOpen ? toggleClose : toggleOpen} size={20} />
            </div>
          </div>
        </div>
      </div>
